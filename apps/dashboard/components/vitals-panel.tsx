...
const RATING_PILL: Record<string, string> = {
  good:               'border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-950/30',
...
export function VitalsPanel({ vitals }: VitalsPanelProps) {
  ...
  return (
    <div className="rounded-md border border-border bg-card px-4 py-3">
      <p className="text-xs font-medium text-muted-foreground mb-2.5">Web vitals · p75</p>
      <div className="flex flex-wrap gap-2">
        {vitals.map((v) => (
          <div
            key={v.name}
            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs ${RATING_PILL[v.rating]}`}
            title={`${v.name} — ${v.total} samples · ${v.good} good · ${v.needsImprovement} needs improvement · ${v.poor} poor\nmedian: ${fmt(v.name, v.median)}  p75: ${fmt(v.name, v.p75)}`}
          >
